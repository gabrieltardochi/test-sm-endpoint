...
resource "aws_sagemaker_model" "sagemaker_model" {
  name               = "${var.endpoint_name}-model-${random_string.solution_suffix.result}"
  execution_role_arn = aws_iam_role.sg_endpoint_role.arn
...
resource "aws_sagemaker_endpoint_configuration" "sagemaker_endpoint_config" {
  name = "${var.endpoint_name}-config-${random_string.solution_suffix.result}"
